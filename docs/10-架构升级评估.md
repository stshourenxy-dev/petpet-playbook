...
| 项 | viewer 现状 | 代码位置 |
|---|---|---|
| 动作切换入口 | 已实现：`requestAction(name, source)` 统一仲裁入口 | `src/main.ts`、`src/state-priority.ts` |
| 触发源 | 已实现：菜单/随机/提醒/初始化 四源带 source 标签，菜单/提醒永远允许，随机仅在待机时，init 用于初始播放与回 idle | `onAction`、`startRandomBehavior`、`handleReminderFire` |
| 状态优先级 | 🟡 已定义未生效：显式优先级表（sleep 0 … poop 5，remind 7 预留）已写入 `ACTION_PRIORITY`，但 `auto` 源当前无任何调用点（requestAction 签名不支持），实际仲裁 = menu/reminder/init 放行 + random 仅待机——与重构前守卫等价。优先级比较待未来接入 auto 源后生效 | `state-priority.ts` |
| 一次性 vs 循环 | 🟡 半：ONESHOT 集合已建（当前为空，8 动作全循环），播放层已支持 | `state-priority.ts` |
| 睡眠序列 | 无。sleep 是单一动作，无 yawning→dozing→collapsing→sleeping→waking 迁移 | — |
| 主题 | 已实现：幽灵换肤已删除（无 CSS 消费、无入口、写入路径移除），`pet.json.theme` 字段保留读取 | `main.js`、`main.ts` |
| 宠物包互操作 | 已实现：托盘「📦 导入宠物包」（目录/zip 双通道，校验后装入 pets/），见 [08-接口协议](08-接口协议.md) §1.1；Codex Pet 格式对接仍是文档规格 | docs/08、`viewer/pet-import.cjs` |
...
- `agents/` 30+ 个 AI 工具适配器（各类 AI 工具…）——不是"可插拔"就能搬，领域语义完全不同
...
1. 新增 `state-priority.ts`（约 60 行）：
   ...
   - ONESHOT 集合：**当前为空**（红苕 8 动作全为循环动画 loop=true，无一次性动作），为未来一次性动作（点击反应/喂食反应）预留
   - `requestAction(name, source)` 统一入口：source 分 menu/reminder/random/init/auto——menu/reminder/init 永远执行，random 仅待机时允许（显式化现有守卫），auto 低优先级不打断高优先级
...
3. 删除 `startRandomBehavior` 内的无条件回 idle（`setTimeout(4000)`），改带守卫：仅当“当前仍是本次随机动作”时才回——修复：用户/提醒打断后不再被强制拉回 idle（真 bug）。

**成本**：约 1 小时编码 + 1 小时回归（8 动作逐个验证 + 提醒打断测试）。**状态**：`state-priority.ts` + `main.ts` 改造已完成（vitest 单测覆盖，见 `tests/state-priority.test.ts`），build 通过。
