...
| 项 | viewer 现状 | 代码位置 |
|---|---|---|
| 动作切换入口 | ✅ 已修：`requestAction(name, source)` 统一仲裁入口 | `src/main.ts`、`src/state-priority.ts` |
| 触发源 | ✅ 已修：菜单/随机/提醒/初始化 四源带 source 标签，菜单/提醒永远允许，随机仅在待机时，init 用于初始播放与回 idle | `onAction`、`startRandomBehavior`、`handleReminderFire` |
...
| 主题 | ✅ 已修：幽灵换肤已删除（无 CSS 消费、无入口、写入路径移除），`pet.json.theme` 字段保留读取 | `main.js`、`main.ts` |
...
**成本**：约 1 小时编码 + 1 小时回归（8 动作逐个验证 + 提醒打断测试）。**P0 已落地（2026-08-08）**：state-priority.ts + main.ts 改造完成，vitest 单测 9 例全绿（tests/state-priority.test.ts），build 通过，未新增 TS 类型错误（5 个遗留错误待白天清理）。回归验证（8 动作 + 提醒打断）与 Windows 封装验证待白天完成。
